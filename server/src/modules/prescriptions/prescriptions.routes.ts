  ...
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.prescription.findMany({
        where,
        include: { medications: true, patient: { include: { user: true } }, doctor: { include: { user: true } } },
        skip: (page - 1) * limit,
        take: limit,
...
prescriptionsRouter.get('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const p = await prisma.prescription.findUnique({
      ...
      include: { medications: true, patient: { include: { user: true } }, doctor: { include: { user: true, department: true } }, appointment: true },
    });
  ...
  try {
    ...
    const updated = await prisma.prescription.update({
      ...
      data: {
        ...rest,
        ...(medications && {
          medications: { deleteMany: {}, create: medications },
        }),
      },
